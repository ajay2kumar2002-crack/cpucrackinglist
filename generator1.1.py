...
class MultiProcessWordlistGenerator:
    ...
    def estimate_file_size(self):
        ...
        for word_count in range(1, max_words + 1):
            ...
            avg_word_len = sum(len(w) for w in self.base_words) / num_words
            estimated_letters = int(avg_word_len * word_count * 0.8)  # 80% are letters
            theoretical_caps = 2 ** estimated_letters
            actual_caps = min(theoretical_caps, self.config['max_capitalizations'])
            
            total_combinations += perms * num_word_seps * total_dates * num_date_seps * actual_caps
        
        # Estimate size (avg 20 chars per password + newline)
        estimated_size_bytes = total_combinations * 21
